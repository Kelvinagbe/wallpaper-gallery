...
export default function SearchPage({ params }: { params?: { query?: string } }) {
  ...
  const decoded = params?.query ? decodeURIComponent(params.query) : '';
  const hasQuery = decoded.length > 0;
  ...
  // Load hot wallpapers (discovery mode)
  useEffect(() => {
  ...
  // Load search results when query param present
  useEffect(() => {
    if (!hasQuery) { setResLoading(false); return; }
    setInput(decoded);
    let cancelled = false;
    (async () => {
      setResLoading(true);
  ...
  // Sentinel for pinned bar (discovery only)
  useEffect(() => {
  ...
  // Measure fixed header height (results mode)
  useEffect(() => {
  ...
  const saveAndGo = (q: string) => {
    ...
    try {
      ...
      localStorage.setItem('recentSearches', JSON.stringify(
        [t, ...prev.filter((s: string) => s.toLowerCase() !== t.toLowerCase())].slice(0, 6)
      ));
    } catch {}
    startLoader();
  ...
  const searchBar = (hero = false) => (
    ...
    >
      ...
      <input
        ...
        style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: 15, color: '#0a0a0a', fontFamily: 'inherit', fontWeight: hero ? 500 : 600, minWidth: 0 }}
      />
      {input.trim() && (
      ...
      <button className="go-btn" onClick={() => saveAndGo(input)}
        style={{ height: hasQuery ? 34 : 36, padding: '0 16px', borderRadius: 11, background: '#0a0a0a', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
        Go
      </button>
    </div>
  );
  ...
  if (hasQuery) {
    return (
      <div style={{ minHeight: '100dvh', background: '#fff', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#0a0a0a' }}>
          ...
          {!resLoading && (
            <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px 10px' }}>
              <p style={{ fontSize: 12, color: '#aaa', fontWeight: 500 }}>
                {results.length > 0
                  ? <><span style={{ fontWeight: 700, color: '#0a0a0a' }}>{results.length}</span> results for <span style={{ fontWeight: 700, color: '#0a0a0a' }}>"{decoded}"</span></>
        ...
        {/* Body */}
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${headerH}px 16px 56px` }}>
