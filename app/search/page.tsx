...
const CSS = `
  ...
  /* ── Masonry grid for wallpaper results ── */
  .search-grid {
    columns: 2;
    column-gap: 8px;
  }
...
export default function SearchPage() {
  ...
  // ── Load recent searches + autofocus ────────────────────────
  useEffect(() => {
    ...
    <div style={{ minHeight: '100dvh', background: '#fff', fontFamily: 'system-ui,sans-serif', color: '#0a0a0a' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', maxWidth: 720, margin: '0 auto' }}>
          ...
          {/* Search input */}
          <div
            style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(0,0,0,0.04)', borderRadius: 14, border: '1px solid transparent', transition: 'border-color .15s' }}
          ...
          >
            ...
            {query && (
              <button onClick={handleClear}
                style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(0,0,0,0.2)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', padding: '0 16px', maxWidth: 720, margin: '0 auto' }}>
          {(['wallpapers', 'people'] as Tab[]).map(t => (
            <button key={t} onClick={() => handleTab(t)}
              style={{ flex: 1, padding: '10px 0', background: 'none', border: 'none', borderBottom: `2px solid ${tab === t ? '#0a0a0a' : 'transparent'}`, fontSize: 13, fontWeight: tab === t ? 700 : 500, color: tab === t ? '#0a0a0a' : 'rgba(0,0,0,0.4)', cursor: 'pointer', fontFamily: 'inherit', transition: 'all .15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px' }}>
        ...
        {!query ? (
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  <button onClick={() => { setRecent([]); localStorage.removeItem('recentSearches'); }}
                    style={{ fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.4)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                    Clear
                  </button>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {recent.map((term, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px 7px 14px', background: 'rgba(0,0,0,0.04)', borderRadius: 24, border: '1px solid rgba(0,0,0,0.07)' }}>
                      <button onClick={() => handleInput(term)} style={{ fontSize: 13, color: '#0a0a0a', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
                        {term}
                      </button>
          ...
          <div className="fade-up">
            ...
            {tab === 'wallpapers' ? (
              // ── Masonry grid — fixes title/info outside image layout ──
              ...
              <div style={{ display: 'flex', flexDirection: 'column', background: '#fafafa', borderRadius: 16, border: '1px solid rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                {people.map((user, i) => (
                  <button key={user.id} className="user-row"
                    onClick={() => { startLoader(); router.push(`/profile/${user.id}`); }}
          ...
          <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 10, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.38)', margin: 0 }}>Try "{query}" with different keywords</p>
          </div>
        )}
      </div>
    </div>
