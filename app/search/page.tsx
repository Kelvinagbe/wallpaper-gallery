...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .search-wrap-hero:focus-within   { box-shadow: 0 0 0 3px rgba(255,255,255,0.55) !important; }
  ...
  .back-btn-float:hover  { background:rgba(255,255,255,0.97) !important; }
  .back-btn-float:active { transform:scale(.95); }
...
export default function SearchPage() {
  ...
  // Sync input field when ?q= changes (back/forward nav)
  useEffect(() => { setInput(query); }, [query]);

  // Hot wallpapers for hero + collections
  useEffect(() => {
    const cached = getCache();
    if (cached?.length) { setHotWalls(cached); setHotLoading(false); return; }
    fetchHotWallpapers(10)
      .then(d => { setCache(d); setHotWalls(d); })
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { wallpapers } = await searchWallpapers(query);
        if (!cancelled) setResults(wallpapers);
      } catch {
  ...
  // Recent searches + autofocus on discovery
  useEffect(() => {
  ...
  // Hero auto-rotate (discovery only)
  useEffect(() => {
  ...
  // Sentinel → pin search bar when hero scrolls away (discovery only)
  useEffect(() => {
  ...
  // Track fixed header height (results mode)
  useEffect(() => {
  ...
  // Push ?q= param — no page reload, single component stays mounted
  ...
  const searchBar = (hero = false) => (
    <div
      className={hero ? 'search-wrap-hero' : 'search-wrap'}
      style={{
    ...
    >
      ...
      />
      {/* In results mode: show active query as a dismissible chip */}
      {hasQuery && (
        <button
          onClick={clearQuery}
          style={{
            display: 'flex', alignItems: 'center', gap: 5,
            height: 28, padding: '0 10px 0 12px', borderRadius: 20,
            background: '#f0f0f0', border: 'none', cursor: 'pointer',
            fontSize: 12, fontWeight: 600, color: '#444', fontFamily: 'inherit',
            flexShrink: 0, maxWidth: 130,
          }}
        >
          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 88 }}>{query}</span>
          <X size={10} color="#888" strokeWidth={2.5} style={{ flexShrink: 0 }} />
        </button>
      )}
      {/* In discovery mode: plain clear button when typing */}
      {!hasQuery && input.trim() && (
        <button
        ...
        <div ref={headerRef} style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: '#fff', borderBottom: '1px solid #f0f0f0', paddingTop: 'env(safe-area-inset-top, 0px)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <button className="back-btn" onClick={goBack}
              style={{ width: 44, height: 44, flexShrink: 0, borderRadius: 14, background: '#f5f5f5', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
          ...
          {resLoading ? (
            ...
            <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', gap: 16, textAlign: 'center' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 14, color: '#bbb', maxWidth: 260, lineHeight: 1.5 }}>Try shorter or different keywords for "{query}"</p>
              </div>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#fff', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#0a0a0a' }}>
      ...
      {/* Pinned bar — appears after hero scrolls away */}
      {pinned && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, paddingTop: 'env(safe-area-inset-top, 0px)', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <button className="back-btn-float" onClick={goBack}
