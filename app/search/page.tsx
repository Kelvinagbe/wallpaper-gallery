...
// ── Each tag gets a real matching Unsplash image ───────────────
const TRENDING_TAGS: { label: string; img: string }[] = [
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .back-btn { transition: background .15s, transform .1s; }
    ...
    <div style={{ minHeight: '100dvh', background: '#fff', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#0a0a0a' }}>
      ...
      <div style={{ position: 'relative', height: 'min(60vw, 420px)', minHeight: 300, overflow: 'hidden', background: '#1a1a1a' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 16px 22px', zIndex: 10 }}>
          ...
          {/* Search bar — Go button always visible */}
          <div
            ...
            />
            {/* X button only when there's text */}
            {query.trim() && (
              <button
      ...
      <div style={{ background: '#fff' }}>
        ...
        {/* Trending Tags — each with its own matching image */}
            ...
            {TRENDING_TAGS.map(({ label, img }, i) => {
              ...
              return (
                <div key={label} className="tag-card"
                  ...
                  style={{ borderRadius: 16, overflow: 'hidden', minHeight: tall ? 120 : 88, position: 'relative', background: '#e8e8e8', border: '1px solid #ebebeb' }}>
