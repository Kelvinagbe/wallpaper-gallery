...
export default function WallpaperDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Fetch related
      const { data: related } = await supabase.from('wallpapers').select('*').neq('id', id).limit(4).order('created_at', { ascending: false });
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <style>{`@keyframes scaleIn{0%{transform:scale(0)}50%{transform:scale(1.2)}100%{transform:scale(1)}}@keyframes successBounce{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}@keyframes spin{to{transform:rotate(360deg)}}.scale-in{animation:scaleIn .4s cubic-bezier(.34,1.56,.64,1)}.success-bounce{animation:successBounce .5s ease-out}.spinner{animation:spin .6s linear infinite}@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}.shimmer{background:linear-gradient(90deg,#ffffff0a 25%,#ffffff14 50%,#ffffff0a 75%);background-size:200% 100%;animation:shimmer 1.5s infinite}`}</style>
      ...
      <div className="max-w-2xl mx-auto p-4 pt-16 pb-8">
        ...
        <div className="relative rounded-2xl overflow-hidden bg-zinc-900 mb-4">
          {pageLoading ? (
            // Skeleton for image - no flashing, just shimmer
            ...
            {pageLoading ? (
              ...
              <>
                <button onClick={() => router.push(`/user/${wallpaper!.userId}`)} className="flex items-center gap-3 flex-1 hover:bg-white/5 rounded-xl transition-colors">
                  <img src={wallpaper!.userAvatar} alt={wallpaper!.uploadedBy} className="w-10 h-10 rounded-full border border-white/20 flex-shrink-0 object-cover" />
                ...
                {state.dataLoading
                  ? <div className="w-20 h-8 shimmer rounded-full" />
                  : <button onClick={handleFollow} className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-all active:scale-95 flex-shrink-0 ${state.following?'bg-white/10 text-white border border-white/20':'bg-white text-black hover:bg-gray-200'}`}>{state.following?'Following':'Follow'}</button>
                }
              </>
            )}
          </div>
