...
export default function WallpaperDetail({ initialWallpaper }: { initialWallpaper: Wallpaper }) {
  ...
  const [counts, setCounts] = useState({ likes: wp.likes, downloads: wp.downloads, views: wp.views });
  ...
  const handleSave = () => auth('save wallpapers', async () => {
    ...
    navigator.vibrate?.(50);
    const key = `${wp.id}-${session!.user.id}`, cached = cache.get(key);
    if (cached) cache.set(key, { ...cached, saved: v, timestamp: Date.now() });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setSt(s => ({ ...s, downloading: false, downloaded: true }));
      navigator.vibrate?.(100);
    } catch {
      setSt(s => ({ ...s, downloading: false }));
    }
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#fff' }}>
      ...
      {/* ── Image ── */}
      <div className="relative w-full overflow-hidden transition-all duration-300" style={{ height: full ? '100dvh' : '50vh', borderRadius: full ? 0 : '0 0 28px 28px', background: '#f3f4f6' }}>
        ...
        <Image
          ...
          onLoad={e => { const i = e.currentTarget as HTMLImageElement; imgCache.add(wp.url); setImgLoaded(true); }}
        />
        {imgLoaded && <>
          {counts.likes > 0 && (
          ...
          <div className="flex items-center gap-3 mb-5">
            <button onClick={() => router.push(`/user/${wp.userId}`)} className="flex items-center gap-3 flex-1 bg-transparent border-none cursor-pointer p-0 text-left">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[13px]" style={{ color: '#9ca3af' }}>{timeAgo}</span>
              </div>
            </button>
            {st.dataLoading
              ? <div className={`w-20 h-9 rounded-full ${SH}`} />
              : <button onClick={handleFollow} className="px-5 py-2 rounded-full text-sm font-semibold flex-shrink-0 transition-all active:scale-95" style={st.following ? { border: '1.5px solid #e5e7eb', color: '#6b7280', background: 'transparent' } : { background: '#111', color: '#fff' }}>
