...
// ── Tokens ────────────────────────────────────────────────────────────────────
const T = { bg:'#f0f2f5', surface:'#fff', border:'rgba(0,0,0,.1)', ink:'#050505', ink2:'#65676b', ink3:'rgba(5,5,5,.3)', black:'#050505', tile:'#e4e6eb', green:'#42b72a', red:'#e41e3f', amber:'#f59e0b' };
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes stickyIn{from{opacity:0;transform:translateY(-56px)}to{opacity:1;transform:translateY(0)}}
  ...
  .sticky-in{position:fixed;top:0;left:0;right:0;width:100%;z-index:150;animation:stickyIn .28s cubic-bezier(.16,1,.3,1) both}
`;
...
const navBar: React.CSSProperties = { background:'#050505', display:'flex', alignItems:'center', gap:10, padding:'0 12px', height:56, borderBottom:'1px solid rgba(255,255,255,.08)', boxShadow:'0 2px 12px rgba(0,0,0,.3)' };
const backBtn: React.CSSProperties = { width:36, height:36, borderRadius:'50%', border:'none', background:'rgba(255,255,255,.12)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0 };
...
export default function UserProfilePage() {
  ...
  const loadProfile = useCallback(async (force = false) => {
    ...
    if (cp) { setProfile(cp); if (cs) setStats(cs); if (cf !== undefined) setIsFollowing(cf); setPageLoading(false); if (!cacheIsStale(profileKey, PROFILE_TTL) && !force) return; }
    try {
      ...
      if (session && !isOwn && (cf === undefined || force)) { const f = await checkIsFollowing(session.user.id, userId); cacheSet(followKey, f); setIsFollowing(f); }
  ...
  const loadPosts = useCallback(async (force = false) => {
    ...
    if (cp) { setPosts(cp.wallpapers); setHasMore(cp.hasMore); setPostsLoading(false); if (!cacheIsStale(postsKey, POSTS_TTL) && !force) return; }
    if (!cp) setPostsLoading(true);
    try { const { wallpapers, hasMore: more } = await fetchUserWallpapers(userId, 0, PAGE_SIZE); cacheSet(postsKey, { wallpapers, hasMore: more }); setPosts(wallpapers); setHasMore(more); setPage(0); }
  ...
  return (
    <div style={{ fontFamily:font, background:T.bg, minHeight:'100dvh', color:T.ink, maxWidth:680, margin:'0 auto', paddingBottom:40 }}>
      ...
      {showSticky && (
        <div key={stickyKey} className="sticky-in" style={navBar}>
          <button className="tap" onClick={() => router.back()} aria-label="Go back" style={backBtn}>
            <ChevronLeft size={20} color="#fff" strokeWidth={2.5}/>
          </button>
          <div style={{ width:34, height:34, borderRadius:'50%', overflow:'hidden', border:'2px solid rgba(255,255,255,.3)', flexShrink:0 }}>
            <Image src={profile.avatar} alt={profile.name} width={34} height={34} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
          </div>
          <span style={{ fontFamily:font, fontSize:17, fontWeight:700, color:'#fff', letterSpacing:'-.01em', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            {profile.name}
          </span>
        </div>
      )}

      {/* ── Static top nav (scrolls away with hero) ── */}
      <div ref={heroRef} style={navBar}>
        <button className="tap" onClick={() => router.back()} aria-label="Go back" style={backBtn}>
          <ChevronLeft size={20} color="#fff" strokeWidth={2.5}/>
        </button>
      </div>
            ...
            <div style={{ padding:'20px 16px 0', display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div style={{ position:'relative', flexShrink:0 }}>
                <div style={{ width:80, height:80, borderRadius:'50%', padding:2.5, background:T.black, boxShadow:'0 0 0 2px #fff' }}>
                  <Image src={profile.avatar} alt={profile.name} width={80} height={80} draggable={false} onContextMenu={e=>e.preventDefault()} style={{ width:'100%', height:'100%', borderRadius:'50%', objectFit:'cover', display:'block' }}/>
                </div>
              ...
              <>
                ...
                {hasMore && (
                  <div style={{ display:'flex', justifyContent:'center', padding:'20px 0 40px' }}>
                    <button className="tap" onClick={loadMore} disabled={!isOnline} style={{ padding:'11px 32px', borderRadius:10, border:`1px solid ${T.border}`, background:T.surface, color:T.ink2, fontFamily:font, fontSize:13, fontWeight:600, cursor:isOnline?'pointer':'default', opacity:isOnline?1:0.4 }}>Load more</button>
                  </div>
                )}
              </>
            ) : (
