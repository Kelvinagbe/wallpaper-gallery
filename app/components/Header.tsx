...
const CSS = `
  ...
  .wl-input:focus,.mob-input:focus{
...
const Overlay = ({ closing, onClick }: { closing: boolean; onClick: () => void }) => (
  <div className={closing ? 'bd-out' : 'bd-in'} onClick={onClick}
...
const CloseBtn = ({ onClick }: { onClick: () => void }) => (
  <button onClick={onClick} style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(0,0,0,0.06)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
export const Header = ({ filter, setFilter, onMenuOpen, startLoader }: HeaderProps) => {
  ...
  const closer = (setShow: (v: boolean) => void, setClosing: (v: boolean) => void, ms: number) =>
    (cb?: () => void) => { setClosing(true); setTimeout(() => { setShow(false); setClosing(false); cb?.(); }, ms); };

  const closeAuth    = closer(setShowAuth,    setClosingAuth,    220);
  const closeSheet   = closer(setShowSheet,   setClosingSheet,   260);
  const closeSidebar = closer(setShowSidebar, setClosingSidebar, 240);
  ...
  return (
    <>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: HDR_BG, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: HDR_BORDER }}>
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '0 16px', height: 54, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div className="desk" style={{ flex: 1, maxWidth: 560, position: 'relative' }}>
            ...
            <input className="wl-input" type="text" placeholder="Search wallpapers, colors, moods…"
          ...
          <div className="mob" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button className="hdr-icon-btn" aria-label="Search" onClick={() => mobOpen ? closeMob() : setMobOpen(true)}>
              {mobOpen && !mobClosing ? <X size={18} color="rgba(255,255,255,0.75)" /> : <Search size={18} color="rgba(255,255,255,0.65)" />}
            </button>
        ...
        {/* Mobile search dropdown — bg matches header */}
        {(mobOpen || mobClosing) && (
          <div className={`mob ${mobClosing ? 'ms-out' : 'ms-in'}`}
      ...
      {(showSidebar || closingSidebar) && (
        <>
          <Overlay closing={closingSidebar} onClick={() => closeSidebar()} />
          <aside className={closingSidebar ? 'sb-out' : 'sb-in'}
            ...
            <div style={{ padding: '18px 16px', borderBottom: '1px solid rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Logo dark /><CloseBtn onClick={() => closeSidebar()} />
            </div>
          ...
          <div className={closingAuth ? 'm-out' : 'm-in'} onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: '28px 26px 30px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <Logo dark /><CloseBtn onClick={() => closeAuth()} />
              </div>
