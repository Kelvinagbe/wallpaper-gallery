...
// ─── Three-dot nav loader ─────────────────────────────────────────────────────
let _showNavLoader: ((v: boolean) => void) | null = null;

const NavLoader = () => {
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    _showNavLoader = setVisible;
    return () => { _showNavLoader = null; };
  }, []);
...
const BottomSheet = ({ isOpen, onClose, wp, saved, onSave, onDownload, onShare }: any) => {
  ...
  return createPortal(
    <>
      <div
        style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', backdropFilter:'blur(8px)', zIndex:100, animation:'fadeIn 0.2s' }}
        onClick={onClose}
      />
...
export const WallpaperCard = ({ wp, onClick }: WallpaperCardProps) => {
  ...
  // ← The only change: show loader dots then navigate
  ...
  return (
    <>
      {/* One NavLoader per card — only the active one is ever visible */}
      <NavLoader />
        ...
        >
          ...
          <div className="relative w-full" style={{ paddingBottom: '125%' }}>
            ...
            <Image
              ...
              style={{ opacity: state.loaded ? 1 : 0, transition: state.loaded ? 'none' : 'opacity 0.3s' }}
              priority={false}
              loading="lazy"
            />
          </div>
          ...
          {/* Hover overlay */}
          <div
            style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(0,0,0,0.8),transparent,transparent)', opacity:0, transition:'opacity 0.2s' }}
