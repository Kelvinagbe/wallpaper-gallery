...
export const BottomSheetProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const items = sheet ? [
    {
      icon: sheet.saved ? BookmarkCheck : Bookmark,
      label: sheet.saved ? 'Saved' : 'Save',
      sub: sheet.saved ? 'Remove from your collection' : 'Add to your collection',
      onClick: sheet.onSave,
      accent: sheet.saved,
      danger: false,
    },
    {
        ...
        <>
          <div
            ...
            style={{
              ...
              background: out ? 'rgba(0,0,0,0)' : 'rgba(0,0,0,0.35)',
              backdropFilter: out ? 'blur(0px)' : 'blur(2px)',
              ...
              <div style={{ padding: '4px 12px 0' }}>
                <button
                  ...
                  style={{
                    ...
                    background: '#f3f4f6', fontSize: 14, fontWeight: 600,
...
export const NativeAdCard = ({ ad, placeholderIndex = 0 }: { ad: Ad; placeholderIndex?: number }) => {
  ...
  return (
    /* ── single wrapper div — flex: '1 1 0' so it matches sibling card widths ── */
    <div style={{ flex: '1 1 0', minWidth: 0 }}>
      ...
      >
        ...
        <div style={{
          ...
          aspectRatio: '9/16', background: hasImg ? ph.bg : gradBg,
        }}>
          {!imgLoaded && !imgError && ad.imageUrl && (
            <div style={{
              position: 'absolute', inset: 0, zIndex: 1,
              background: `linear-gradient(105deg,transparent 40%,${ph.shimmer}99 50%,transparent 60%)`,
              backgroundSize: '200% 100%', animation: 'shimmerSweep 1.8s ease-in-out infinite',
            }} />
          )}

          {ad.imageUrl && (
            <img
              src={ad.imageUrl} alt={ad.title} draggable={false}
...
export const WallpaperCard = ({ wp, onClick, priority = false, placeholderIndex = 0 }: {
  ...
  return (
    /* ── single wrapper div — this is the flex child in the row ── */
    <div style={{ flex: '1 1 0', minWidth: 0 }}>
      ...
      <div
        style={{
          position: 'relative',
          width: '100%',
          aspectRatio: '9/16',       // always maintains ratio; no contain needed
          borderRadius: 16,
          overflow: 'hidden',
          background: ph.bg,
          cursor: 'pointer',
      ...
      >
        ...
        {loaded && isPC && (
          <div style={{
            ...
            background: 'rgba(0,0,0,.5)', fontSize: 9, fontWeight: 700,
        ...
        {loaded && (
          ...
          >
            <MoreHorizontal style={{ width: 14, height: 14, color: '#fff' }} />
          </button>
        )}
      </div>

      {/* text below image — always reserve space to avoid layout shift */}
      <div style={{ padding: '5px 0 0', display: 'flex', flexDirection: 'column', gap: 3, minHeight: 36 }}>
        {loaded && (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
              ...
              >
                <Heart
                  style={{
                    width: 11, height: 11, transition: 'all .15s',
                    fill: liked ? '#f43f5e' : 'none',
                    color: liked ? '#f43f5e' : 'rgba(0,0,0,.3)',
                    transform: liked ? 'scale(1.1)' : 'scale(1)',
                  }}
                />
                {likeCount > 0 && (
                  <span style={{ fontSize: 10, fontWeight: 500, color: liked ? '#f43f5e' : 'rgba(0,0,0,.3)' }}>
                    {fmt(likeCount)}
                  </span>
                )}
              </button>
