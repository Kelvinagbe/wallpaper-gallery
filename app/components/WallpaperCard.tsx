...
/* ── re-export ad cards so the grid only needs one import source ── */
export { NativeAdCard, BannerAdCard } from './AdCards';
...
/* ════════════════════════════════════════
   BOTTOM SHEET
════════════════════════════════════════ */
const BottomSheet = ({ isOpen, onClose, wp, saved, onSave, onShare }: {
  isOpen: boolean; onClose: () => void; wp: Wallpaper; saved: boolean; onSave: () => void; onShare: () => void;
}) => {
  const [visible, setVisible] = useState(false);
  const [out, setOut] = useState(false);

  useEffect(() => {
    if (isOpen) { setOut(false); setVisible(true); }
    else if (visible) { setOut(true); const t = setTimeout(() => setVisible(false), 280); return () => clearTimeout(t); }
  }, [isOpen]);
  ...
  return createPortal(
    <>
      <div onClick={close} style={{ position: 'fixed', inset: 0, background: out ? 'rgba(0,0,0,0)' : 'rgba(0,0,0,0.35)', backdropFilter: out ? 'blur(0px)' : 'blur(2px)', zIndex: 100, transition: 'background .28s ease,backdrop-filter .28s ease' }} />
...
export const WallpaperCard = ({ wp, onClick, priority = false, placeholderIndex = 0 }: {
  ...
  const [s, setS] = useState({ loaded: imgCache.has(imgSrc), liked: false, saved: false, showMenu: false });
  const [likeCount, setLikeCount] = useState(wp.likes || 0);
  const upd = (p: Partial<typeof s>) => setS(prev => ({ ...prev, ...p }));
  ...
  const onTouchEnd  = useCallback(() => { if (longTimer.current) clearTimeout(longTimer.current); }, []);
  const onTouchMove = useCallback(() => { if (longTimer.current) clearTimeout(longTimer.current); }, []);
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    const next = !s.liked;
    upd({ liked: next }); setLikeCount(c => next ? c + 1 : Math.max(0, c - 1));
    navigator.vibrate?.(50);
    try { await toggleLike(wp.id); }
    catch { if (mounted.current) { upd({ liked: !next }); setLikeCount(c => next ? Math.max(0, c - 1) : c + 1); } }
  };

  const handleSave = async () => {
    if (!user) return alert('Please login to save wallpapers');
    const next = !s.saved;
    upd({ saved: next, showMenu: false }); navigator.vibrate?.(50);
    try { await toggleSave(wp.id); }
    catch { if (mounted.current) upd({ saved: !next }); }
  };
  ...
  return (
    <>
      <div className="relative w-full rounded-2xl overflow-hidden cursor-pointer select-none"
        ...
        onClick={handleClick} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        onTouchMove={onTouchMove} onContextMenu={noCtx}
      >
        {!s.loaded && (
        ...
        <Image src={imgSrc} alt={wp.title} fill draggable={false}
          ...
          onLoad={() => { imgCache.add(imgSrc); if (mounted.current) upd({ loaded: true }); }}
          className="object-cover z-[2]" priority={priority} loading={priority ? 'eager' : 'lazy'}
          style={{ opacity: s.loaded ? 1 : 0, transition: 'opacity .4s ease', pointerEvents: 'none', WebkitUserDrag: 'none' } as React.CSSProperties}
          onContextMenu={e => e.preventDefault()}
        />

        {s.loaded && isPC && (
          <div className="absolute top-2 left-2 z-[5]" style={{ padding: '2px 7px', borderRadius: 5, background: 'rgba(0,0,0,.5)', backdropFilter: 'blur(6px)', fontSize: 9, fontWeight: 700, color: '#fff', letterSpacing: '.06em', textTransform: 'uppercase' }}>
            Desktop
          </div>
        )}

        {s.loaded && (
          <button aria-label="More options" onClick={e => { e.stopPropagation(); upd({ showMenu: true }); }} className="absolute top-2 right-2 z-[5] p-1.5 rounded-full bg-black/30 backdrop-blur-sm">
        ...
        <div style={{ padding: '6px 2px 0', display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
            ...
            <button aria-label="Like" onClick={handleLike} style={{ display: 'flex', alignItems: 'center', gap: 3, background: 'none', border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0 }}>
              <Heart style={{ width: 11, height: 11, transition: 'all .15s' }} className={s.liked ? 'fill-rose-500 text-rose-500 scale-110' : 'text-gray-300'} />
              {likeCount > 0 && <span style={{ fontSize: 10, fontWeight: 500, color: s.liked ? '#f43f5e' : 'rgba(0,0,0,.3)' }}>{fmt(likeCount)}</span>}
            </button>
          </div>
        </div>
      )}

      <BottomSheet isOpen={s.showMenu} onClose={() => upd({ showMenu: false })} wp={wp} saved={s.saved} onSave={handleSave} onShare={handleShare} />

      <style>{`@keyframes shimmerSweep{0%{background-position:-200% 0}100%{background-position:200% 0}}img{-webkit-user-drag:none}`}</style>
    </>
  );
};
