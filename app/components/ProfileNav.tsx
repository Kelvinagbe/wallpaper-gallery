...
export const ProfileNav = ({ onClose, wallpapers, onWallpaperClick }: ProfileNavProps) => {
  ...
  const displayStats = [
    { label: 'Posts', value: loading ? null : formatCount(stats.posts || myWallpapers.length) },
    { label: 'Followers', value: loading ? null : formatCount(stats.followers) },
    { label: 'Following', value: loading ? null : formatCount(stats.following) },
  ];
              ...
              {myWallpapers.length > 0 && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {myWallpapers.map(wp=>(
                      <div key={wp.id} onClick={()=>onWallpaperClick(wp)} className="relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer hover:opacity-80 active:scale-95 transition-all">
