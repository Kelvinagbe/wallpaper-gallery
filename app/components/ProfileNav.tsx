...
export const ProfileNav = ({ onClose, wallpapers, onWallpaperClick }: ProfileNavProps) => {
  ...
  if (loading) return <div className="fixed inset-0 bg-black z-50 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-4 border-white/20 border-t-white"></div></div>;
  if (!profile) return <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-center p-4"><h2 className="text-2xl font-bold mb-4">Please Log In</h2><p className="text-white/60 mb-6">You need to be logged in to view your profile</p><button onClick={handleClose} className="px-6 py-3 bg-white text-black rounded-full font-semibold hover:bg-gray-200 active:scale-95 transition-all">Go Back</button></div>;
  ...
  return (
    <>
      ...
      {modals.allPosts&&<ViewAllPostsModal onClose={()=>setModals(m=>({...m,allPosts:false}))} wallpapers={wallpapers.filter(wp=>wp.userId===profile.id)} onWallpaperClick={wp=>{setModals(m=>({...m,allPosts:false}));onWallpaperClick(wp);}} userName={profile.name}/>}
