    ...
    <a href={ad.ctaUrl} target="_blank" rel="noopener noreferrer sponsored" onClick={() => trackClick(ad.id)} style={{ display: 'block', textDecoration: 'none' }}>
      <div style={{ position: 'relative', width: '100%', borderRadius: 16, overflow: 'hidden', aspectRatio: '9/16', background: hasImage ? ph.bg : gradBg }}>
        ...
        <div style={{ position: 'absolute', bottom: 10, left: 8, right: 8, zIndex: 4, display: 'flex', alignItems: 'center', gap: 7 }}>
          <BrandMark ad={ad} size={24} radius={7} border="1.5px solid rgba(255,255,255,.2)" />
    ...
    <div style={{ padding: `8px ${horizontalPadding}px` }}>
      <a href={ad.ctaUrl} target="_blank" rel="noopener noreferrer sponsored" onClick={() => trackClick(ad.id)} style={{ display: 'block', textDecoration: 'none' }}>
        <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', minHeight: 86, background: bg, boxShadow: '0 2px 20px rgba(0,0,0,.14),0 0 0 1px rgba(255,255,255,.04) inset' }}>
          ...
          {ad.imageUrl && (
            <img src={ad.imageUrl} alt="" aria-hidden draggable={false}
              onLoad={() => setImgLoaded(true)} onError={() => setImgError(true)}
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: imgLoaded ? 1 : 0, transition: 'opacity .5s ease', pointerEvents: 'none', zIndex: 0 }}
            />
          )}

          <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: hasImage ? 'linear-gradient(90deg,rgba(0,0,0,.78) 0%,rgba(0,0,0,.48) 55%,rgba(0,0,0,.2) 100%)' : `linear-gradient(135deg,${bg} 0%,${bg}dd 100%)` }} />
