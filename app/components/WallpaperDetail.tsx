            ...
            {(isLoading || state.dataLoading) && !wallpaper ? <SkeletonLoader /> : (
              <>
                ...
                <div className="space-y-4 mt-4">
                  ...
                  {relatedWallpapers.length > 0 && <div><h4 className="font-semibold text-white mb-3">Related</h4><div className="grid grid-cols-2 gap-3">{relatedWallpapers.map(wp => <button key={wp.id} onClick={() => onRelatedClick(wp)} className="relative aspect-[9/16] rounded-xl overflow-hidden hover:scale-[1.02] transition-transform active:scale-95"><img src={wp.thumbnailUrl || wp.url} alt={wp.title} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent" /></button>)}</div></div>}
                </div>
              </>
            )}
          </div>
