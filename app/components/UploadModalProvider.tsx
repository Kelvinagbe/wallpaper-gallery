...
const S = {
  ...
  input: {
    width: '100%', padding: '11px 14px', background: 'rgba(0,0,0,0.03)',
    border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, fontSize: 14,
    color: '#0a0a0a', fontFamily: 'inherit', outline: 'none', transition: 'border-color .15s',
  } as React.CSSProperties,
};
...
const UploadModal = ({ onClose }: { onClose: () => void }) => {
  ...
  const {
    uploading, progress, status, error,
    online, speed, canResume,
    uploadFile, reset, cancel, getCachedData,
  } = useUpload(user?.id);
      ...
      >
        ...
        {uploading && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 10, borderRadius: '24px 24px 0 0', background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '32px 24px', animation: 'upl-overlay-in .25s ease forwards' }}>
            ...
            {/* Icon */}
            {isComplete
              ? <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'upl-pop .4s cubic-bezier(.34,1.56,.64,1) forwards' }}>
          ...
          {authLoading ? (
            ...
            <>
              ...
              <div>
                {/* Trigger */}
                <button
                  onClick={() => !uploading && setCatOpen(p => !p)}
                  disabled={uploading}
                  style={{ ...S.input, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: uploading ? 'default' : 'pointer', color: category ? '#0a0a0a' : 'rgba(0,0,0,0.35)' }}
                >
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, padding: '2px 0 4px' }}>
                    {CATEGORIES.map(cat => {
                      ...
                      return (
                        <button
                          key={cat}
                          onClick={() => { setCategory(active ? '' : cat); setCatOpen(false); }}
