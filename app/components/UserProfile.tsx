...
import { getProfile, getUserStats, checkIsFollowing, toggleFollow, getLiked, getSaved } from '@/lib/stores/userStore';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (currentUser) {
          // Check if following
          const followStatus = await checkIsFollowing(currentUser.id, userId);
          setIsFollowing(followStatus);

          // If viewing own profile, load liked and saved
          if (currentUser.id === userId) {
            ...
            // Extract wallpapers arrays from the returned objects
            setLikedWallpapers(likedData.wallpapers);
            setSavedWallpapers(savedData.wallpapers);
          }
        }
      } catch (error) {
